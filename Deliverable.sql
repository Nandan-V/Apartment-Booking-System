...
/* 
 * Task 3: Queries for the database system 
 * Alias names used:
 * Buildings: "B"
 * Apartment: "A"
 * Booking: "BK"
 ...
 * Description: Retrieves all apartments in Building 111 with details like type, apartment number, rent, and availability status.
 */
 ...
 *
 * Explanation:
 * - LEFT JOIN: Ensures that apartments with no bookings still appear in the results.
 * - Condition: Filters apartments that are available (Availability = TRUE) and have no overlapping bookings.
 */
SELECT A.ApartmentID, A.Type, A.ApartmentNumber, A.Rent, A.Availability
FROM   Apartment A
LEFT JOIN   Booking BK 
    ON A.ApartmentID = BK.ApartmentID 
   AND (BK.CheckInDate <= '2024-01-10' AND BK.CheckOutDate >= '2024-01-01')
WHERE A.Availability = TRUE AND BK.ApartmentID IS NULL;

/* 3. Count the number of booked apartments for each building
 * 
 * Explanation:
 * - JOIN: Connects Buildings, Apartments, and Bookings to ensure accurate aggregation.
 * - COUNT: Tallies the number of apartments linked to a booking in each building.
 */
